function [trainImgMatrix,testImgMatrix] = readData(dirpath,dbType,downSample)
    if strcmp(dbType,'att_faces')
        dim=[112,92];
        [trainImgMatrix,testImgMatrix]=readDB(dirpath,dim,32,6,4,downSample);
    end
    if strcmp(dbType,'yale')
        dim=[192,168];
        [trainImgMatrix,testImgMatrix]=readDB(dirpath,dim,38,40,20,downSample);
    end
    if strcmp(dbType,'ExtYale')
        dim=[480,640];
        [trainImgMatrix,testImgMatrix]=readDB(dirpath,dim,28,10,10,downSample);
    end
end

% Reads the images from the Database and returns the trainImage dataset and
% test image data set.
function [trainImgCell,testImgCell]=readDB(dirpath,dimension,numOfPerson,trainSize,testSize,downSample)
